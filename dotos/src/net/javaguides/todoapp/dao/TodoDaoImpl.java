...
public class TodoDaoImpl implements TodoDao {
	...
	private static final String INSERT_TODOS_SQL = "INSERT INTO todos"
			+ "  (plan, title, username, description, target_date,  is_done) VALUES " + " (?,?, ?, ?, ?, ?);";

	private static final String SELECT_TODO_BY_ID = "select id,plan,title,username,description,target_date,is_done from todos where id =?";
	private static final String SELECT_ALL_TODOS = "select * from todos";
	private static final String DELETE_TODO_BY_ID = "delete from todos where id = ?;";
	private static final String UPDATE_TODO = "update todos set plan = ?, title = ?, username= ?, description =?, target_date =?, is_done = ? where id = ?;";
	...
	public void insertTodo(Todo todo) throws SQLException {
		...
		try (Connection connection = JDBCUtils.getConnection();
			...
			preparedStatement.setString(1, todo.getPlan());
	...
	public Todo selectTodo(long todoId) {
		...
		try (Connection connection = JDBCUtils.getConnection();
			...
			while (rs.next()) {
				...
				todo = new Todo(id, plan, title, username, description, targetDate, isDone);
			}
		} catch (SQLException exception) {
			JDBCUtils.printSQLException(exception);
		}
	...
	public List<Todo> selectAllTodos() {
		...
		try (Connection connection = JDBCUtils.getConnection();
			...
			while (rs.next()) {
				...
				LocalDate targetDate = rs.getDate("target_date").toLocalDate();
				boolean isDone = rs.getBoolean("is_done");
				todos.add(new Todo(id, plan, title, username, description, targetDate, isDone));
			}
		} catch (SQLException exception) {
			JDBCUtils.printSQLException(exception);
		}
	...
	public boolean updateTodo(Todo todo) throws SQLException {
		...
		try (Connection connection = JDBCUtils.getConnection();
			...
			statement.setString(1, todo.getPlan());
